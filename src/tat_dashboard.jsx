...
// ============ CALCULATION ============
function calcForSlot(dealer, hubKey, params, slot, slotIndex, anchorMode, kmOverride, flightOverride, routeFlightOverrides, liveRouteFlights, liveDriveTimes, liveFlightHours) {
  const city = CITY_AIRPORT[dealer.city];
  if (!city) return null;
  const hub = HUBS[hubKey];
  const destApt = city.code;

  // Distance + drive hours: Google Routes data is the source of truth.
  // If the user overrides km, scale Google hours proportionally to stay realistic.
  const live = liveDriveTimes && liveDriveTimes[dealer.id];
  const liveKm  = live ? live.km    : null;
  const liveHrs = live ? live.hours : null;
  const km = kmOverride != null
    ? kmOverride
    : (liveKm != null ? liveKm : (DEALER_KM[dealer.id] != null ? DEALER_KM[dealer.id] : city.km));
  let driveHrs;
  if (liveHrs != null) {
    driveHrs = (kmOverride != null && liveKm > 0)
      ? liveHrs * (kmOverride / liveKm)
      : liveHrs;
  } else {
    driveHrs = null;
  }
  ...
  // Flight duration: user override > live AeroAPI 7-day avg > hardcoded FLIGHT_HRS
  const liveFh = liveFlightHours && liveFlightHours[routeKey];
  const flightHrs = flightOverride != null
    ? flightOverride
    : (liveFh != null ? liveFh : FLIGHT_HRS[hub.airport][destApt]);
  ...
  // Airport → dealer drive time: prefer real Google Routes hours; fallback ~25 km/h average if missing
  const dealerDriveHrs = driveHrs != null ? driveHrs : (km / 25);
...
function calcAllSlots(dealer, hubKey, params, anchorMode, kmOverride, flightOverride, routeFlightOverrides, liveRouteFlights, liveDriveTimes, liveFlightHours) {
  const sorted = [...params.slots].sort((a,b) => a.start - b.start);
  return sorted.map((s, idx) => ({
    slotId: s.id, slotStart: s.start, slotEnd: s.end,
    ...calcForSlot(dealer, hubKey, params, s, idx, anchorMode, kmOverride, flightOverride, routeFlightOverrides, liveRouteFlights, liveDriveTimes, liveFlightHours)
  })).filter(r => r.destApt);
}
...
export default function Dashboard() {
  ...
  const [liveFlightHours, setLiveFlightHours] = useState(null);
  const [liveRouteMeta, setLiveRouteMeta] = useState(null);
      ...
      .then(json => {
        ...
        if (json.routes) {
          for (const [key, r] of Object.entries(json.routes)) {
            ...
            if (r.avg_flight_hours != null) hours[key] = r.avg_flight_hours;
            meta[key] = {
              frequency:     r.frequency,
              frequencyNote: r.frequency_note,
              daysObserved:  r.days_observed,
              daysWindow:    r.days_window,
              avgFlightHrs:  r.avg_flight_hours
            };
          }
        }
        setLiveRouteFlights(flat);
        setLiveFlightHours(hours);
  ...
  const results = useMemo(() => DEALERS.map(d => {
    ...
    return {
      dealer: d,
      bhiwandi: calcAllSlots(d, 'bhiwandi', params, anchorMode, ov.km, ov.flightBom, routeFlightOverrides, liveRouteFlights, liveDriveTimes, liveFlightHours),
      sriperumbudur: calcAllSlots(d, 'sriperumbudur', params, anchorMode, ov.km, ov.flightMaa, routeFlightOverrides, liveRouteFlights, liveDriveTimes, liveFlightHours)
    };
  }), [params, overrides, anchorMode, routeFlightOverrides, liveRouteFlights, liveDriveTimes, liveFlightHours]);
